...
/**
 * Created by KUBRAMEVLUT on 24.04.2018.
 */

public class SorguAraclari {

    long timeInMillisecons;
    ...
    /**
     * Create a private constructor because no one should ever create a {@link SorguAraclari} object.
     * This class is only meant to hold static variables and methods, which can be accessed
     * directly from the class name SorguAraclari (and an object instance of SorguAraclari is not needed).
     */
    private SorguAraclari() {
    }

    /**
     * Return a list of {@link Deprem} objects that has been built up from
     * parsing a JSON response.
     */
    public static ArrayList<Deprem> DepremVeriAyikla() {

        // Create an empty ArrayList that we can start adding earthquakes to
        ArrayList<Deprem> depremler = new ArrayList<>();
        ...
        try {
            ...
            JSONArray DepremArray = baseJsonResponse.getJSONArray("features");

            for (int i = 0; i < DepremArray.length(); i++) {
                JSONObject currentDeprem = DepremArray.getJSONObject(i);
                JSONObject properties = currentDeprem.getJSONObject("properties");

                double buyukluk = properties.getDouble("mag");
                String konum = properties.getString("place");
                Long zaman = properties.getLong("time");
                String websayfasi = properties.getString("url");

                Deprem deprem = new Deprem(buyukluk, konum, zaman, websayfasi);
            ...
            Log.e("SorguAraclari", "Problem parsing the earthquake JSON results", e);
        }
